...
'''
내 코드는 2가지 조합까지만 된다는 단점이 있음
이는 for문이 두개이기 떄문이긴한데 일단은 해설 코드를 해석하는 방향으로 가겠음

import time
import datetime

# memoization 방식으로 풀어라 -> 재귀 함수 사용
def max_profit_memo(price_list, count, cache):
    num = 0

    for i in range(len(price_list)):
        for j in range(len(price_list)):
            if i + j == count and price_list[i] + price_list[j] not in cache.values():
                cache[num] = price_list[i] + price_list[j]
                num += 1
